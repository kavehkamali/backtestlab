...
function MetricCard({ label, value, suffix = '', icon: Icon, positive }) {
  const color = positive === undefined ? 'text-[var(--eq-accent)]'
    : positive ? 'text-[var(--eq-gain)]' : 'text-[var(--eq-loss)]';
  return (
    <div className="bg-[var(--eq-card)] shadow-sm ring-1 ring-[var(--eq-border)] rounded-xl p-4">
      <div className="flex items-center justify-between mb-1">
        <span className="text-xs text-[var(--eq-text3)] uppercase tracking-wider">{label}</span>
...
function CustomTooltip({ active, payload, label }) {
  ...
  return (
    <div className="bg-[var(--eq-card)] ring-1 ring-[var(--eq-border)] rounded-lg px-3 py-2 text-xs">
...
export default function ResultsPanel({ result, loading }) {
  if (loading) {
    return (
      <div className="flex items-center justify-center h-96 text-[var(--eq-text3)]">
  ...
  if (!result) {
    return (
      <div className="flex items-center justify-center h-96 text-[var(--eq-text2)] text-sm">
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Equity Curve */}
      <div className="bg-[var(--eq-card)] shadow-sm ring-1 ring-[var(--eq-border)] rounded-xl p-4">
        <h3 className="text-sm font-medium text-[var(--eq-text3)] mb-4">Equity Curve</h3>
      ...
      {/* Drawdown Chart */}
      <div className="bg-[var(--eq-card)] shadow-sm ring-1 ring-[var(--eq-border)] rounded-xl p-4">
        <h3 className="text-sm font-medium text-[var(--eq-text3)] mb-4">Drawdown</h3>
      ...
      {monthly_returns.length > 0 && (
        <div className="bg-[var(--eq-card)] shadow-sm ring-1 ring-[var(--eq-border)] rounded-xl p-4">
          <h3 className="text-sm font-medium text-[var(--eq-text3)] mb-4">Monthly Returns</h3>
      ...
      {trades.length > 0 && (
        <div className="bg-[var(--eq-card)] shadow-sm ring-1 ring-[var(--eq-border)] rounded-xl p-4">
          <h3 className="text-sm font-medium text-[var(--eq-text3)] mb-4">
              ...
              <tbody>
                {trades.slice(-50).map((t, i) => (
                  <tr key={i} className="border-b border-[var(--eq-grid)] hover:bg-[var(--eq-card2)]">
                    <td className="py-1.5 px-2 text-[var(--eq-text3)]">{t.date}</td>
                    <td className={`py-1.5 px-2 font-medium ${t.type === 'buy' ? 'text-[var(--eq-gain)]' : 'text-[var(--eq-loss)]'}`}>
                      {t.type.toUpperCase()}
                    </td>
                    <td className="py-1.5 px-2 text-right text-[var(--eq-text2)]">${t.price}</td>
                    <td className="py-1.5 px-2 text-right text-[var(--eq-text3)]">{t.shares}</td>
                    <td className={`py-1.5 px-2 text-right font-medium ${t.pnl >= 0 ? 'text-[var(--eq-gain)]' : 'text-[var(--eq-loss)]'}`}>
