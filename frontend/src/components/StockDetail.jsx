...
function StatRow({ label, value, color }) {
  return (
    <div className="flex justify-between py-1 border-b border-[var(--eq-grid)]">
      <span className="text-[var(--eq-text3)]">{label}</span>
      <span className={color || 'text-[var(--eq-text2)]'}>{value ?? '—'}</span>
    </div>
  );
}
...
function PerfBadge({ label, value }) {
  ...
  const color = value > 0 ? 'text-[var(--eq-gain)] bg-emerald-500/10' : value < 0 ? 'text-[var(--eq-loss)] bg-red-500/10' : 'text-[var(--eq-text3)] bg-[var(--eq-card)]/5';
  return (
    <div className={`px-2 py-1 rounded-md text-center ${color}`}>
      <div className="text-[9px] text-[var(--eq-text3)] mb-0.5">{label}</div>
...
function CustomTooltip({ active, payload, label }) {
  ...
  return (
    <div className="bg-[var(--eq-card)] ring-1 ring-[var(--eq-border)] rounded-lg px-3 py-2 text-[10px] shadow-xl">
      <p className="text-[var(--eq-text3)] mb-1">{label}</p>
      {d?.open !== undefined && (
        <>
          <p className="text-[var(--eq-text2)]">O: ${d.open} &nbsp; H: ${d.high}</p>
          <p className="text-[var(--eq-text2)]">L: ${d.low} &nbsp; C: ${d.close}</p>
        </>
      )}
      {d?.volume !== undefined && (
        <p className="text-[var(--eq-text3)]">Vol: {(d.volume / 1e6).toFixed(1)}M</p>
      )}
    </div>
  );
}
  ...
  return (
    <div className="text-[11px] overflow-y-auto max-h-[calc(100vh-320px)]">
      ...
      <div className="px-4 pt-3 pb-2">
        <div className="text-lg font-bold text-[var(--eq-text)]">${data.price}</div>
        <div className="text-[var(--eq-text3)]">{data.name}</div>
        <div className="text-[10px] text-[var(--eq-text2)]">{data.sector} · {data.industry}</div>
      </div>
        ...
        {Object.keys(ranges).map(r => (
          <button
            ...
            className={`px-2 py-0.5 rounded text-[10px] font-medium transition-all ${
              chartRange === r
                ? 'bg-[var(--eq-accent-soft)] text-[var(--eq-accent)]'
                : 'text-[var(--eq-text2)] hover:text-[var(--eq-text3)]'
            }`}
          >
            {r}
          </button>
      ...
      <div className="px-4 py-3 space-y-0.5">
        <div className="text-[10px] text-[var(--eq-text3)] uppercase tracking-wider mb-2">Valuation</div>
      ...
      <div className="px-4 pb-3 space-y-0.5">
        <div className="text-[10px] text-[var(--eq-text3)] uppercase tracking-wider mb-2">Financials</div>
        <StatRow label="Profit Margin" value={data.profit_margin != null ? `${data.profit_margin}%` : '—'} color={data.profit_margin > 0 ? 'text-[var(--eq-gain)]' : 'text-[var(--eq-loss)]'} />
        <StatRow label="Revenue Growth" value={data.revenue_growth != null ? `${data.revenue_growth}%` : '—'} color={data.revenue_growth > 0 ? 'text-[var(--eq-gain)]' : 'text-[var(--eq-loss)]'} />
        <StatRow label="Earnings Growth" value={data.earnings_growth != null ? `${data.earnings_growth}%` : '—'} color={data.earnings_growth > 0 ? 'text-[var(--eq-gain)]' : 'text-[var(--eq-loss)]'} />
      ...
      {/* Ownership */}
      <div className="px-4 pb-3 space-y-0.5">
        <div className="text-[10px] text-[var(--eq-text3)] uppercase tracking-wider mb-2">Ownership</div>
      ...
      {/* Price & Technical */}
      <div className="px-4 pb-3 space-y-0.5">
        <div className="text-[10px] text-[var(--eq-text3)] uppercase tracking-wider mb-2">Price & Technical</div>
        ...
        <StatRow label="RSI (14)" value={data.rsi} color={data.rsi >= 70 ? 'text-[var(--eq-loss)]' : data.rsi <= 30 ? 'text-[var(--eq-gain)]' : undefined} />
        <StatRow label="MACD" value={data.macd} color={data.macd_hist > 0 ? 'text-[var(--eq-gain)]' : 'text-[var(--eq-loss)]'} />
        <StatRow label="MACD Histogram" value={data.macd_hist} color={data.macd_hist > 0 ? 'text-[var(--eq-gain)]' : 'text-[var(--eq-loss)]'} />
      </div>
    </div>
  );
}
