  ...
  return (
    <div>
      <div className="flex items-center justify-between text-xs mb-1">
        ...
        <span className="text-zinc-500 tabular-nums">{sub}</span>
      </div>
...
function CovStat({ label, value, sub }) {
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-3">
      ...
      <div className="text-xl font-bold text-zinc-900 mt-0.5 tabular-nums">{value}</div>
...
export default function AdminDataPlatformTab() {
  ...
  if (loading && !data) {
    return <div className="flex items-center gap-2 text-sm text-zinc-500 py-10 justify-center">
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2 text-sm text-zinc-600">
          ...
          {data?.writer_active && !data?.available && (
            <span className="text-amber-600 text-xs">(warehouse busy — collector holds write lock)</span>
          )}
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={load}
            className="p-1.5 rounded-lg text-zinc-500 hover:text-indigo-600 hover:bg-zinc-100"
            title="Refresh"
          >
      ...
      {/* VPN */}
      <div className={`rounded-xl border p-4 ${vpn.netns_up ? 'border-emerald-200 bg-emerald-50/50' : vpn.configured ? 'border-amber-200 bg-amber-50/40' : 'border-zinc-200 bg-white'}`}>
        <div className="flex items-center gap-2 mb-2">
          {vpn.netns_up ? <Shield className="w-4 h-4 text-emerald-600" /> : <ShieldOff className="w-4 h-4 text-zinc-400" />}
          <span className="font-semibold text-sm text-zinc-800">Collector egress VPN</span>
          <span className={`text-[11px] px-2 py-0.5 rounded-full font-medium ${vpn.netns_up ? 'bg-emerald-500/15 text-emerald-700' : vpn.configured ? 'bg-amber-500/15 text-amber-700' : 'bg-zinc-200 text-zinc-500'}`}>
            {vpn.netns_up ? 'ACTIVE' : vpn.configured ? 'CONFIGURED (netns down)' : 'NOT CONFIGURED'}
          </span>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
          <div><div className="text-zinc-400">Namespace</div><div className="font-medium text-zinc-700">protonvpn</div></div>
          <div><div className="text-zinc-400">Exit IP</div><div className="font-medium text-zinc-700 tabular-nums">{vpn.exit_ip || '—'}</div></div>
          <div><div className="text-zinc-400">Mode</div><div className="font-medium text-zinc-700">{vpn.mode || '—'}</div></div>
          <div><div className="text-zinc-400">Routed</div><div className="font-medium text-zinc-700">{(vpn.routed_collectors || []).length} collectors</div></div>
        </div>
        {(vpn.routed_collectors || []).length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-2">
            {vpn.routed_collectors.map((c) => (
              <span key={c} className="text-[11px] px-2 py-0.5 rounded bg-white border border-zinc-200 text-zinc-600">{c}</span>
            ))}
          </div>
        )}
      </div>
      ...
      <div className="rounded-xl border border-zinc-200 bg-white p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="font-semibold text-sm text-zinc-800">Collector progress</span>
        </div>
        {collectors.length === 0 ? (
          <div className="text-xs text-zinc-500">No collector has reported state yet. Runs on the next timer tick.</div>
        ) : (
      ...
      <div>
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="font-semibold text-sm text-zinc-800">Warehouse coverage</span>
        </div>
      ...
      <div>
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="font-semibold text-sm text-zinc-800">Coverage by asset class</span>
        </div>
        <div className="overflow-x-auto rounded-xl border border-zinc-200">
          <table className="w-full text-xs">
            ...
            <tbody>
              {byClass.map((r) => (
                <tr key={r.asset_class} className="border-t border-zinc-100">
                  <td className="px-3 py-2 font-medium text-zinc-700">{CLASS_LABEL[r.asset_class] || r.asset_class || '—'}</td>
                  <td className="px-3 py-2 text-right tabular-nums">{num(r.symbols)}</td>
                  <td className="px-3 py-2 text-right tabular-nums">
                    {num(r.price_covered)}
                    <span className="text-zinc-400"> ({r.symbols ? Math.round((r.price_covered / r.symbols) * 100) : 0}%)</span>
                  </td>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div>
          <div className="font-semibold text-sm text-zinc-800 mb-2">Recent runs</div>
          <div className="overflow-x-auto rounded-xl border border-zinc-200">
            <table className="w-full text-xs">
              <thead className="bg-zinc-50 text-zinc-500">
                <tr>
              ...
              <tbody>
                {runs.length === 0 ? (
                  <tr><td colSpan={4} className="px-3 py-3 text-center text-zinc-400">No runs recorded yet</td></tr>
                ) : runs.map((r, i) => (
                  <tr key={i} className="border-t border-zinc-100">
                    <td className="px-3 py-2 font-medium text-zinc-700">{r.collector}</td>
                    <td className="px-3 py-2 text-center">
                      {r.ok ? <CheckCircle className="w-3.5 h-3.5 text-emerald-500 inline" /> : <XCircle className="w-3.5 h-3.5 text-red-500 inline" />}
                    </td>
                    <td className="px-3 py-2 text-right tabular-nums">{num(r.rows)}</td>
                    <td className="px-3 py-2 text-right text-zinc-500">{ago(r.finished_at)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div>
          <div className="font-semibold text-sm text-zinc-800 mb-2">Largest covered equities</div>
          <div className="overflow-x-auto rounded-xl border border-zinc-200 max-h-96 overflow-y-auto">
            <table className="w-full text-xs">
              ...
              <tbody>
                {top.length === 0 ? (
                  <tr><td colSpan={4} className="px-3 py-3 text-center text-zinc-400">No profiles collected yet</td></tr>
                ) : top.map((r) => (
                  <tr key={r.symbol} className="border-t border-zinc-100">
                    <td className="px-3 py-2 font-semibold text-zinc-800">{r.symbol}</td>
                    <td className="px-3 py-2 text-zinc-600 truncate max-w-[14rem]">{r.name || '—'}</td>
                    <td className="px-3 py-2 text-zinc-500">{CLASS_LABEL[r.asset_class] || r.asset_class || '—'}</td>
