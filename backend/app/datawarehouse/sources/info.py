...
def _write_rows(sql, rows):
    """Chunked write with a short-lived connection so the single-writer lock is
    held only briefly — collectors don't block each other or the web reader."""
    if not rows:
        return
    con = connect()
    try:
...
def collect_info(symbols: list[str] | None = None, flush_every: int = 20) -> dict:
    started = datetime.utcnow()
    n = 0
    note = ""
    # Read the symbol list + class map with a short connection, then release it.
    con = connect()
    try:
        if symbols is None:
    ...
    for i, sym in enumerate(symbols):
        try:
            info = yf.Ticker(sym).info or {}  # network, no lock held
            if info:
                buf.append([sym, now, info.get("longName") or info.get("shortName"),
...
_QUOTE_SQL = """INSERT INTO prices_daily (symbol,date,open,high,low,close,adj_close,volume,source)
    VALUES (?,?,?,?,?,?,?,?,?)
...
def collect_quotes(symbols: list[str] | None = None, chunk: int = 80) -> dict:
    """Batch latest daily bar for all symbols (cheap, frequent). Lock-light:
    each chunk fetches over the network then writes with a short-lived
    connection, so it never blocks other collectors or the web reader."""
    started = datetime.utcnow()
    total = 0
    if symbols is None:
        con = connect()
